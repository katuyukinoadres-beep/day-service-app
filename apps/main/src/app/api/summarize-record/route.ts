...
export async function POST(request: NextRequest) {
  ...
  const systemPrompt = `あなたは放課後等デイサービスで保護者向けに支援記録を書く支援員です。受け取った記録本文を、保護者視点の温度感・丁寧語・その子らしさのニュアンスを保ったまま指定字数以内に圧縮します。

圧縮ルール:
- 「です・ます」調を厳守
- 文章構造（段落・空行）を可能な限り維持する。活動が複数ある場合は活動ごとに段落を保つ
- 具体的なエピソードの核（何に取り組んだか、どう頑張ったか）は必ず残す
- 形容詞・副詞の重複や冗長な接続を削る
- 見出し・箇条書き・括弧書き・注釈は一切追加しない
- 最終行が「担当：○○」の署名で終わる場合はそのまま末尾に保持する
- 圧縮結果の出力は本文テキストのみ。前置きや説明文を付けない`;

  const userPrompt = `以下の記録本文を ${targetChars} 字以内に圧縮してください。段落構造と温度感を保ってください。
