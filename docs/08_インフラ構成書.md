...
| デプロイ方式 | Git 連携 or CLI (`npx vercel --prod`) |
| プロジェクト構成 | メインアプリ（day-service-app）+ Admin（day-service-admin）の2プロジェクト |
| カスタムドメイン | 設定可能 |
...
| `ANTHROPIC_API_KEY` | 開発用キー | 本番用キー | AI文章生成用（メインアプリのみ） |
| `SUPABASE_SERVICE_ROLE_KEY` | 開発用キー | 本番用キー | RLSバイパス用（Adminのみ） |
| `ADMIN_PASSWORD` | 開発用PW | 本番用PW | Admin認証用（Adminのみ） |
...
| サブドメイン | 用途 | 指向先 |
|---|---|---|
| `app.example.com` | メインアプリ | Vercel (day-service-app) |
| `admin.example.com` | Admin管理画面 | Vercel (day-service-admin) |
