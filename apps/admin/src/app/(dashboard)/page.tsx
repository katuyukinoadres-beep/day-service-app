...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [statsRes, recordsRes] = await Promise.all([
        ...
        fetch("/api/admin-data?type=recent-records"),
      ]);
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-foreground md:text-2xl">ダッシュボード</h1>

      <div className="mb-8 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6 lg:gap-4">
        <StatsCard title="施設数" value={stats?.total_facilities ?? 0} color="#1B6B4A"
        ...
        <StatsCard title="ユーザー数" value={stats?.total_users ?? 0} color="#3B82F6"
          icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" /></svg>} />
        <StatsCard title="児童数" value={stats?.total_children ?? 0} color="#8B5CF6"
        ...
        <StatsCard title="総記録数" value={stats?.total_records ?? 0} color="#F59E0B"
        ...
        <StatsCard title="本日の記録" value={stats?.records_today ?? 0} color="#EF4444"
        ...
        <StatsCard title="本日稼働施設" value={stats?.facilities_with_activity_today ?? 0} color="#10B981"
          icon={<svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>} />
      </div>
      ...
      <div className="rounded-lg bg-white shadow-sm border border-border">
        <div className="border-b border-border px-4 py-3 md:px-6 md:py-4">
          <h2 className="text-base font-semibold text-foreground md:text-lg">最近の記録</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[600px]">
            <thead>
              <tr className="border-b border-border bg-gray-50">
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-sub md:px-6">児童名</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-sub md:px-6">日付</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-sub md:px-6">気分</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-sub md:px-6">活動</th>
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-sub md:px-6">作成日時</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {recentRecords.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-sub md:px-6">記録がありません</td></tr>
              ) : (
                recentRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    <td className="whitespace-nowrap px-4 py-4 text-sm font-medium text-foreground md:px-6">{record.children?.name ?? "—"}</td>
