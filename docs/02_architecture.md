...
>
> v1.1.0 spec-reset: 2026-04-18 に NiKo ビジョンへの整合を目的として、3層アーキテクチャ（A/B/C）を明示。B 層（施設管理者ダッシュボード）は未実装、`apps/facility-admin` として追加予定。
...
| AI生成 | `@anthropic-ai/sdk` | - | Claude Opus 4.7 APIによる支援記録文章の自動生成 |
...
### 2.1 システムコンテキスト図（3層構成）

NiKo ビジョンに基づく3層アーキテクチャ。A（ユーザー）/ B（施設管理者ダッシュボード）/ C（システムAdmin）。B は Phase 1.2 以降で追加予定。

```
[スタッフ/保護者]  --HTTPS--> [A: ユーザーアプリ (apps/main)]
                                       |
                                  REST API / SDK
                                       |
                                  [Supabase]
...
[事業所長(上田くん)] --HTTPS--> [B: 施設管理者ダッシュボード (apps/facility-admin)]
                                                      |
                                                 (🔲 未構築 / Phase 1.2で追加)
                                                      |
                                                 [Supabase](同DB、RLS下の自施設データ)

[スーパー管理者]   --HTTPS--> [C: システムAdminアプリ (apps/admin)]
                                       |
                              service_role key
                              (RLSバイパス)
                                       |
...
[A/Bアプリ] --HTTPS--> [Anthropic API (Claude Opus 4.7)]
                       AI支援記録まとめ生成
```
...
モノレポ（npm workspaces）で構成されたディレクトリ構造を示す。Phase 1.2 で `apps/facility-admin`（B層）を追加予定。

```
day-service-app/
├── apps/
│   ├── main/                      # A層: ユーザーアプリ（スタッフ・将来保護者向け）
│   │   └── src/
│   │       ├── app/
│   │       │   ├── layout.tsx     # ルートレイアウト
...
│   │
│   ├── facility-admin/            # B層: 施設管理者ダッシュボード（🔲 Phase 1.2 で追加予定、未構築）
│   │   └── ...                    # 可視化＋意思決定支援（KPI・児童別ビュー・請求準備など）
│   │
│   └── admin/                     # C層: システムAdminアプリ（スーパー管理者向け）
│       └── src/
│           ├── app/
│           │   ├── login/         # Adminログイン
...
| パス | アプリ | 用途 | 備考 |
|------|--------|------|------|
| `/api/generate-record` | main | AI支援記録まとめ生成 | `ANTHROPIC_API_KEY` を使用。Claude Opus 4.7 |
...
| Anthropic API | AI支援記録まとめ生成 | API Key (Bearer) | 従量課金 | Claude Opus 4.7。RPMの制限あり |
...
### ADR-005: 3層アーキテクチャ（A/B/C）への再整理

- **状況**: 2026-04-18 NiKo 引渡パッケージ受領により、Phase 1.1 の実証施設（上田くん）の要件が明確化。事業所長は「入力者ではなく判断者」であり、A（ユーザー入力）と C（システム管理）とは別に、B（施設管理者ダッシュボード）が必要
- **決定**: アプリ層を A（ユーザー）/ B（施設管理者ダッシュボード）/ C（システムAdmin）の3層に整理。既存 `apps/main` = A、`apps/admin` = C。B は新規に `apps/facility-admin` として Phase 1.2 で追加
- **理由**: 事業所長の使用シーンは「可視化＋意思決定」であり、スタッフの入力 UI とは求めるものが根本的に異なる。同じアプリに統合すると両者の UX が中途半端になる。一方、システムAdmin（全施設横断）とも権限・RLS 戦略が異なるため分離する
- **影響**:
  - Vercel プロジェクトが2から3に増える（将来）
  - B は A/C と同じ Supabase を使用し、RLS 下で自施設データのみアクセス（C の `service_role` とは異なる）
  - `packages/shared` を B からも使用する（型定義・Supabase クライアント共通化）

### ADR-006: 活動項目を固定enumから施設カスタマイズ可能なマスタテーブルに変更

- **状況**: Phase 1.1 実証施設（上田くん）の紙フォームには「眼球運動／宿題／漢字トレーニング（詳細）／計算トレーニング（詳細）／その他取り組み（詳細）」という**施設固有のカリキュラム**が記載されている。従来の固定 enum 9項目（工作/運動/学習/...）では対応不可
- **決定**: `activities: text[]` を廃止し、`activity_items` マスタテーブル ＋ 記録側に `daily_record_activities` 連結テーブルを新設。施設ごとにカスタマイズ可能、`has_detail_field` フラグで詳細記入欄の有無を指定
- **理由**: ①紙フォーム忠実度の確保（NiKo原則7「紙に見た目を寄せる」）、②Q6 I（将来のメニュー追加・変更）対応、③施設ごとのカリキュラム差分を吸収、④集計（Phase 1.2 B層ダッシュボード）で項目別統計が取りやすい
- **影響**:
  - DBマイグレーション必要（新テーブル、既存データの移行／もしくは廃止後に新規入力のみ対応）
  - API 変更（活動マスタ CRUD の新設、記録 API のペイロード形式変更）
  - UI 変更（記録入力画面、活動マスタ管理画面の新設）
  - 詳細 §4.2 / `03_database.md` の「Phase 1.1 予定スキーマ」参照
