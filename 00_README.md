...
| 項目 | 値 |
|------|---|
| 本番URL（A: ユーザーアプリ） | https://day-service-app.vercel.app |
| 本番URL（C: システムAdmin） | https://day-service-admin.vercel.app |
| 将来URL（B: 施設管理者ダッシュボード） | 未構築（Phase 1.2 で追加予定） |
| リポジトリ | https://github.com/{owner}/day-service-app |
| 技術スタック | Next.js 16 / React 19 / TypeScript 5 / Supabase / Tailwind CSS 4 / PWA |
| AI | Anthropic Claude API（Opus 4.7、支援記録生成） |
| ホスティング | Vercel |
| ステータス | 開発中（Phase 1.1 NiKo ビジョン整合のため再構築中） |
| バージョン | v1.1.0 予定（[CHANGELOG.md](CHANGELOG.md) 参照、現在 Unreleased） |
| 設計哲学 | 「放デイ職員の手間を最小化し、子供と向き合う時間を最大化する」 |
...
```
day-service-app/
├── apps/main/              # A層: ユーザーアプリ（@patto/main）
├── apps/facility-admin/    # B層: 施設管理者ダッシュボード（🔲 Phase 1.2 で追加予定）
├── apps/admin/             # C層: システムAdmin管理画面（@patto/admin）
...
| ドキュメント | 対象 | 用途 |
|-------------|------|------|
| [SETUP_GUIDE.md（導入マニュアル）](docs/SETUP_GUIDE.md) | 施設長（一度きり） | 新規導入時のセットアップ手順 |
| [USER_MANUAL.md（取り扱い説明書）](docs/USER_MANUAL.md) | 現場職員 + 代表者 | 日々の操作・トラブル対応 |
