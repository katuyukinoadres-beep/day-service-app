...
export default function NewFacilityPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/admin-data?type=create-facility", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      setError("施設の作成に失敗しました: " + (data.error ?? "不明なエラー"));
      setLoading(false);
      return;
    }
  ...
  return (
    <div>
      <h1 className="mb-6 text-xl font-bold text-foreground md:text-2xl">施設を新規作成</h1>

      <div className="max-w-2xl rounded-lg bg-white shadow-sm border border-border p-4 md:p-6">
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div>
            ...
            <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)}
              placeholder="○○放課後デイサービス" required
          ...
          <div>
            <label htmlFor="plan" className="mb-1.5 block text-sm font-medium text-foreground">プラン</label>
          ...
          <div>
            <label htmlFor="notes" className="mb-1.5 block text-sm font-medium text-foreground">備考</label>
